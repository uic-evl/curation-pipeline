...
class PDFigCapX():
    ...
    def extract(self, _input_path, _output_path):
        xpdf_output_path_prefix = join(
            _output_path, PDF_OUTPUT_FOLDER)  # /.../xpdf_
        ...
        try:
            ...
            pdfs = [x for x in listdir(_input_path) if x[0] != '_' and '.pdf' in x.lower(
            ) and os.path.isfile(join(_input_path, x))]

            for pdf in pdfs:
                total_pdfs += 1
                pdf_path = join(_input_path, pdf)

                pil_images = renderer.render_pdf(
    ...
    def __convert_pdf_to_html(self, _xpdf_output_path, _pdf, _pdf_path):
        try:
            # xpdf_{pdf_name_without_extension}
    ...
    def __extract_figures(self, _input_path, _pdf, _xpdf_output_path):
        ...
        while flag == 0 and wrong_count < MAX_WRONG_COUNT:
            try:
                ...
                self.log_file.write("ERROR: %s\t%s\n%s" % (
                    _pdf, "_extract_figures (%d/%d)".format(wrong_count, MAX_WRONG_COUNT), e))
